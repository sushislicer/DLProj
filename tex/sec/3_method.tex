...
\subsection{Transformer layer parameterization}
We focus on decoder-only Transformer blocks.
Each block contains self-attention projections
$\W_q,\W_k,\W_v,\W_o$ and an MLP with projections
$\W_{\mathrm{up}},\W_{\mathrm{gate}},\W_{\mathrm{down}}$.
In our implementation, these are the \texttt{target\_modules} for adapter insertion and (optionally) rotation-aware quantization.
...
\method decomposes adaptation into three stages and makes explicit the \emph{decomposition between a low-rank update and a quantized residual base}.
For each targeted weight matrix $\W$, we conceptually write
\begin{equation}
  \W \approx \W_{\mathrm{res}} + \Delta\W, \qquad \mathrm{rank}(\Delta\W)\le r.
  \label{eq:residual_decomp}
\end{equation}
The adapter parameters represent $\Delta\W$ while $\W_{\mathrm{res}}$ is the residual/base component that will be quantized.
In the codebase, PiSSA produces PEFT adapter weights and a saved ``residual model'' checkpoint that is subsequently quantized.

Concretely, the stages are:
\begin{enumerate}
...
Let $\W \approx \bm{U}_r \Sigma_r \bm{V}_r^\top$ be the rank-$r$ approximation.
One convenient factorization is
\begin{equation}
  \B_0 = \bm{U}_r\,\Sigma_r^{1/2}, \qquad \A_0 = \Sigma_r^{1/2}\,\bm{V}_r^\top,
  \label{eq:pissa_init}
\end{equation}
...
\paragraph{Architecture-specific insertion.}
Adapters are inserted into attention and MLP projections (e.g., \texttt{q\_proj}, \texttt{k\_proj}, \texttt{v\_proj}, \texttt{o\_proj}, \texttt{gate\_proj}, \texttt{up\_proj}, \texttt{down\_proj}).
This choice matches common PEFT practice and concentrates capacity in the highest-impact linear maps.
...
To keep runtime bounded, we default to a blockwise Hadamard transform
$\R=\mathrm{diag}(\bm{H},\bm{H},\dots)$ where $\bm{H}\bm{H}^\top=\I$ and $\bm{H}$ is the normalized Hadamard matrix.
This acts as a cheap mixing transform that reduces per-dimension variance and typically lowers quantization error.

\paragraph{Quantization objective view.}
SpinQuant can be seen as choosing $\R$ to minimize a rotated-space reconstruction error such as
\begin{equation}
  \min_{\R\in \mathcal{O}(d_{\mathrm{in}})}\; \|\Q_b(\W\R) - \W\R\|_F^2,
  \label{eq:spinquant_obj}
\end{equation}
where $\mathcal{O}(d)$ is the orthogonal group.
Our Hadamard backend fixes $\R$ to a structured orthogonal matrix to avoid the per-layer optimization loop.
...
In mixed-precision and checkpointed training, we ensure that checkpointed blocks receive at least one input with \texttt{requires\_grad=True}, which is required by \texttt{torch.utils.checkpoint}.
